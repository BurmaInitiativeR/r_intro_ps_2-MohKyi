...
## a sample size 10 and replication time 100.

```{r mean_list_1}
library(tidyverse)
df<- iris
set.seed(211132)
means_list <- replicate(n = 100, 
                          expr = mean(
...
## a sample size 30 and replication time 200.

```{r mean_list__2}
set.seed(211132)
means_list <- replicate(n = 200, 
                          expr = mean(
                            sample(df$Sepal.Length, 
                                   size = 30, 
                                   replace=TRUE)))
...
mean_list_2 <- data.frame(cbind(test, means_list)) 
```

## a sample size 50 and replication time 1,000.

```{r mean_list_3}
set.seed(211132)
means_list <- replicate(n = 1000, 
                          expr = mean(
                            sample(df$Sepal.Length, 
                                   size = 50, 
                                   replace=TRUE)))
...
mean_list_3 <- data.frame(cbind(test, means_list))
```


## a sample size 50 and replication time 3,000.

```{r mean_list_4}
set.seed(211132)
means_list <- replicate(n = 3000, 
                          expr = mean(
                            sample(df$Sepal.Length, 
                                   size = 50, 
                                   replace=TRUE)))
...
mean_list_4 <- data.frame(cbind(test, means_list)) 
```

## a sample size 50 and replication time 10,000.

```{r mean_list_5}

set.seed(211132)
means_list <- replicate(n = 10000, 
                          expr = mean(
                            sample(df$Sepal.Length, 
                                   size = 50, 
                                   replace=TRUE)))
...
mean_list_5 <- data.frame(cbind(test, means_list)) 
```
...
6. calculate the mean of `means_list_x` (where `x` `1:5`) from that list (using the command related to filtering list from lecture 2. 

```{r}
mean(means_series[[1]]$means_list)
mean(means_series[[2]]$means_list)
mean(means_series[[3]]$means_list)
mean(means_series[[4]]$means_list)
mean(means_series[[5]]$means_list)
```

7. Using `bind_rows()` function to combined all dataset from `means_seris` and assigned into object called `df_means_combined`. Please make sure that your result dataset should have `16100` observations and 2 variables; `test` and `mean_list`. 
...
8. Finally, calculate the mean value for each group of `test` using following example. In this exercise, we are going to use `group_by` function from the `tidyverse` packge and `%>%` operator (pipe operator). 
